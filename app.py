...
class User(UserMixin, db.Model):
    ...
    last_seen = db.Column(db.DateTime, default=get_ist_time)  # ✅ IST
...
class Message(db.Model):
    ...
    timestamp = db.Column(db.DateTime, default=get_ist_time)  # ✅ IST
...
class BlockedUser(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=get_ist_time)  # ✅ IST
...
def logout():
    ...
    user.online = False
    user.last_seen = get_ist_time()  # ✅ IST
    db.session.commit()
    logout_user()
    return redirect("/login")
...
def handle_disconnect():
    if current_user.is_authenticated:
        ...
        current_user.last_seen = get_ist_time()  # ✅ IST
        db.session.commit()
        emit('user_status', {
            'user_id': current_user.id,
            'username': current_user.username,
...
def handle_send_message(data):
    ...
    # ✅ Create message with IST timestamp
    message = Message(
        ...
        is_delivered=True,
        timestamp=get_ist_time()  # ✅ IST timestamp
    )
    ...
    message_data = {
        ...
        'timestamp': message.timestamp.isoformat(),  # ✅ Send IST timestamp
